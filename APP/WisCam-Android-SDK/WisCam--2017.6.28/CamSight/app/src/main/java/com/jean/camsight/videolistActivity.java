...
{   
	...
	private MainMenuButton exit_btn;
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		exit_btn=(MainMenuButton)findViewById(R.id.exit_btn);
		exit_btn.setOnClickListener(new Exit_photolist_Click());
		...
		getFiles(PlayActivity.videofile_path);//获取文件夹下所有视频信息，并添加到列表
	}
						...
						for (int i = line; i >= 0; i--) {
							if (adapter_my.flag[i] == 1) {
								if ((i > 0) && (i < line)) {
									...
									File file = new File(PlayActivity.videofile_path + "/" + delete_photo + ".mp4");
									DeleteFile(file);
									listItem.remove(i);
							  ...
							  public void onClick(DialogInterface dialog,int which)
								{
								  listItem.clear();
						          listItemAdapter.notifyDataSetChanged();
						          File file = new File(PlayActivity.videofile_path);
						          DeleteFile(file); 
								}
							})
