...
public class photolistActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getFiles(PlayActivity.photofile_path);//获取文件夹下所有图片信息，并添加到列表
	}
	@Override
						...
						for (int i = line; i >= 0; i--) {
							if (adapter_my.flag[i] == 1) {
								if ((i > 0) && (i < line)) {
									...
									File file = new File(PlayActivity.photofile_path + "/" + delete_photo + ".jpg");
									DeleteFile(file);
									listItem.remove(i);
							  ...
							  public void onClick(DialogInterface dialog,int which)
								{
								  listItem.clear();
						          listItemAdapter.notifyDataSetChanged();
						          File file = new File(PlayActivity.photofile_path);
						          DeleteFile(file); 
								}
							})
