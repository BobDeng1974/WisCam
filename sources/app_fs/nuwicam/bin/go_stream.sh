...
while [ 1 ]; do
	...
	if  [ -f "/mnt/nuwicam/etc/stream_config" ]; then
		...
		# Set resolution of packet pipe to QVGA
	    # echo "$PATH_ROOT/bin/RTSPServer -x 320 -y 240 -w $STREAM_VINWIDTH -d $STREAM_VINHEIGHT -i $STREAM_JPEGENCWIDTH -e $STREAM_JPEGENCHEIGHT -b $STREAM_BITRATE"
        # $PATH_ROOT/bin/RTSPServer -x 320 -y 240 -w $STREAM_VINWIDTH -d $STREAM_VINHEIGHT -i $STREAM_JPEGENCWIDTH -e $STREAM_JPEGENCHEIGHT -b $STREAM_BITRATE

		# Set resolution of packet pipe to QVGA, and test streaming from packet pipe
        echo "$PATH_ROOT/bin/RTSPServer -x 320 -y 240 -p -w $STREAM_VINWIDTH -d $STREAM_VINHEIGHT -i $STREAM_JPEGENCWIDTH -e $STREAM_JPEGENCHEIGHT -b $STREAM_BITRATE"
        $PATH_ROOT/bin/RTSPServer -x 320 -y 240 -p -w $STREAM_VINWIDTH -d $STREAM_VINHEIGHT -i $STREAM_JPEGENCWIDTH -e $STREAM_JPEGENCHEIGHT -b $STREAM_BITRATE
